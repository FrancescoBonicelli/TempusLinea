...
void Canvas::placeEvents(std::vector<Event*> events_vector, QFontMetrics fm)
{
	...
	int event_start_y = (height() / 2) + v_offset - 50;
...
void Canvas::placePeriods(std::vector<Period*> periods_vector_full, QFontMetrics fm, int v_offset, bool is_collapsed)
{
	...
	for (int i = 0; i < periods_vector.size(); i++)
	{
		...
		while (!placed)
		{
			...
			{
				if ((!is_collapsed &&
					(periods_vector.at(i)->label_rect.adjusted(1, 1, -1, -1).intersects(periods_vector.at(y)->label_rect)
				...
				{
					placed = false;
					periods_vector.at(i)->label_rect.translate(0, 1.1 * period_height);
					periods_vector.at(i)->period_rect.translate(0, 1.1 * period_height);
					break;
				}
			}
		}
